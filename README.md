...
- **Multicast** : un groupe est identifié par son adresse IPv4 multicast, avec nom et description. Chaque groupe contient des flux identifiés par leur port UDP, avec contenu, description, zéro ou plusieurs IP sources déjà attribuées dans G@IP et zéro ou plusieurs VLAN associés. La fiche d’un groupe affiche une colonne par site, dans l’ordre d’affichage, avec ✔ vert si le flux utilise au moins un VLAN du site et ✖ rouge sinon. Un groupe non vide ne peut pas être supprimé.
