...
Application desktop C# / .NET 10 / Avalonia 12.1 pour gérer sites, VLAN, adressage IPv4 et multicast. Sans SQL, serveur, inventaire matériel ou compte applicatif. Windows x64 et Linux x64 ; X11/XWayland par défaut sous Linux, Wayland natif uniquement sur demande explicite. G@IP est un logiciel open source distribué sous licence MIT.

## Compiler, tester, lancer

Prérequis : SDK .NET **10.0.401**, accès NuGet pour la première restauration. Les `packages*.lock.json` versionnés figent les dépendances utilisées par les builds reproductibles.
...
- **Ajouter un site** : code unique, nom, description. **Ajouter un VLAN** : VID, nom et cases multi-sites ; CIDR et passerelle facultatifs par site. Les VLAN créés sont indépendants. Les formulaires d’ajout de VLAN et d’IP proposent **Enregistrer et en ajouter un autre** pour les saisies en série.
- **Multicast** : un groupe est identifié par son adresse IPv4 multicast, avec nom et description. Chaque groupe contient des flux identifiés par leur port UDP, avec contenu, description, zéro ou plusieurs IP sources déjà attribuées dans G@IP et zéro ou plusieurs VLAN associés. Dans la vue d’un groupe, un clic sélectionne un flux, un double-clic l’édite et le clic droit propose Modifier/Supprimer ; ces interactions valent aussi en cliquant sur les cellules ✓/✕. La fiche d’un groupe affiche une colonne par site, dans l’ordre d’affichage, avec ✓ vert si le flux utilise au moins un VLAN du site et ✕ rouge sinon. Dans l’éditeur de flux, sources et VLAN sont séparés en deux parties : les éléments déjà sélectionnés restent regroupés en tête et toujours visibles ; la recherche ne filtre que la partie « Disponibles ». Les listes disponibles restent parcourables sans recherche et sont virtualisées pour ne matérialiser que les lignes visibles, même avec plusieurs milliers d’IP. Cocher un résultat le fait remonter immédiatement dans « Sélectionnés », et le décocher le renvoie dans les disponibles s’il correspond au filtre courant. Un groupe non vide ne peut pas être supprimé. Les créations de groupe et de flux proposent **Enregistrer et en ajouter un autre** ; pour les flux, sources et VLAN sélectionnés sont conservés.
...
- **CSV** : import avec toutes les erreurs détectées avant publication, export VLAN, IP et multicast. Dans `multicast.csv`, les sources sont listées une par ligne dans leur cellule et les VLAN une par ligne sous la forme `SITE/VID — NOM`. Les groupes sans flux restent exportés avec leurs métadonnées ; plusieurs flux de natures différentes peuvent partager la même adresse multicast sur des ports distincts. **Excel** : export `.xlsx` complet avec un onglet Sites et VLAN, un onglet **Multicast** lorsqu’au moins un groupe existe, des liens hypertextes vers un onglet par réseau, les informations réseau et toutes les IP utilisables. L’onglet Multicast reprend adresse, groupe, port, contenu, sources, VLAN et une colonne par site avec ✔ vert / ✖ rouge. **Recherche globale** : inclut aussi groupes/ports multicast, sources et VLAN associés. **Historique** : 1 000 dernières actions, détails dépliables regroupés par objet sans répétition inutile, recherche instantanée et filtrage contextuel d’un groupe multicast.
- Dans une vue VLAN, **CSV** exporte uniquement ce VLAN et ses IP ; **Historique** affiche ses dernières actions liées, y compris les libérations d’IP et les ajouts/retraits de ce VLAN dans les flux multicast, sans exposer les autres VLAN dans les détails. Les ajouts génériques sont accessibles à l’accueil.
...
La base initiale est vide. Pour une démonstration volontaire, importer `samples/vlans.csv`, puis `samples/addresses.csv`, puis `samples/multicast.csv`. Le jeu de démonstration contient 6 sites avec respectivement 5, 7, 9, 11, 13 et 15 VLAN, soit 60 VLAN au total, 6 tailles de sous-réseaux différentes (/23 à /28), 3 470 attributions IP, ainsi que **20 groupes multicast et 258 flux**. Chaque groupe possède entre **5 et 20 ports** ; les associations sont volontairement réparties sur 1 à 4 sites par flux afin d’obtenir un mélange visible de ✓ verts et ✕ rouges dans la matrice multicast.

## Compatibilité du schéma métier

Le format actuel de `gaip-data.json` constitue officiellement le **schéma V1 de référence**. À partir de cette base, toute évolution future incompatible avec un lecteur V1 devra incrémenter `schemaVersion` et prévoir une migration explicite. Les versions antérieures au gel de cette V1 sont considérées comme des versions de développement ; tous les postes partageant une même base doivent rester sur une version compatible.
...
À chaque tag strict `vMAJOR.MINOR.PATCH`, les tests et tous les artefacts Windows/Linux sont d’abord construits et vérifiés. Le RPM est installé réellement sous AlmaLinux 8 en CI. La Release GitHub n’est créée en brouillon puis rendue publique qu’après réussite complète de ces étapes. Elle publie une seule archive portable trimmed par OS, ainsi qu’un **MSI Windows x64**, un **DEB amd64** et un **RPM x86_64**. Tous les artefacts sont issus du même binaire Release trimmed.
...
Ou `./scripts/publish.ps1` / `sh scripts/publish.sh` : **Release** génère une seule publication officielle par RID dans `artifacts/Release/<RID>/`. Elle est autonome, single-file, compressée et trimmed (`PublishTrimmed=true`, `TrimMode=full`). Il n’existe plus de variante Release standard non trimée ni de profil `ExperimentalTrimmed`.

Le profil **Development** reste autonome à fichiers séparés avec symboles dans `artifacts/Development/<RID>/` et n’est pas trimmed. Voir [les profils et vérifications de publication](docs/PUBLICATION.md) et [les mesures historiques de taille](docs/PUBLICATION_SIZE.md).

Linux nécessite les bibliothèques graphiques usuelles : fontconfig, EGL/OpenGL, Wayland ; X11/XWayland et bibliothèques associées pour le repli. Les publications x64 sélectionnent les packages et services Avalonia par RID : Win32 sous Windows, X11 par défaut sous Linux, Skia et HarfBuzz dans les deux cas. Les compilations sans RID conservent `UsePlatformDetect()`. Sous Linux, G@IP utilise **X11/XWayland par défaut**, y compris dans une session Wayland. Le backend Wayland natif reste disponible uniquement sur demande explicite dans une vraie session Wayland :
