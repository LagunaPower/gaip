...
- ExperimentalTrimmed est construite séparément pour `win-x64` et `linux-x64`. Le rapport historique du 19 septembre documente côté Windows **0 avertissement de trimming**, 21,76 Mio et un démarrage natif réussi ; cela ne constitue pas une validation exhaustive de tous les parcours graphiques. Voir [le rapport de taille](PUBLICATION_SIZE.md).
...
| CSV | Création et mise à jour, UTF-8, séparateur alternatif, guillemets, erreurs multiples, doublons, chevauchements, absence d’import partiel, refus de changement de CIDR avec IP attribuées ; jeu de démonstration importable validé : 6 sites avec 5, 7, 9, 11, 13 et 15 VLAN, soit 60 VLAN, 3 470 IP, 6 préfixes différents, 20 groupes multicast et 258 flux de 5 à 20 ports ; chaque flux couvre 1 à 4 sites afin d’exercer les ✓/✕ ; export contextuel par GUID même avec VID identique sur un autre site |
| Excel | Classeur `.xlsx` valide, onglet Sites et VLAN, onglet Multicast avec groupes/flux, sources, VLAN et matrice de sites ✔/✖ dans l’ordre d’affichage, lien hypertexte VLAN → réseau, un onglet par sous-réseau, informations réseau, passerelle/attributions/libres et garde de capacité Excel |
